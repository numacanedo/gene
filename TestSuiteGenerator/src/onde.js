...
onde.Onde = function (formElement, schema, documentInst, opts) {
    ...
    this.panelElement.find('.item-add').live('click', function (evt) {
        ...
        if ($(this.parentElement).hasClass('collapsed')) {
            $(this.parentElement).removeClass('collapsed');
            $('#' + $(this.parentElement).attr('data-fieldvalue-container-id')).slideDown('fast');
        }
        return false;
    });
    this.panelElement.find('.item-copy').live('click', function (evt) {
        evt.preventDefault();
        _inst.onAddListItem($(this));
        return false;
    });
    this.panelElement.find('.item-paste').live('click', function (evt) {
        evt.preventDefault();
        _inst.onAddListItem($(this));
        if ($(this.parentElement).hasClass('collapsed')) {
            $(this.parentElement).removeClass('collapsed');
    ...
    this.panelElement.find('.collapser').live('click', function (evt) {
        ...
        if (fieldId) {
            ...
            if (collapser.hasClass('collapsed')) {
                ...
                if(collapser[0].childNodes[collapser[0].childNodes.length-1].localName == null){
                    collapser[0].childNodes[collapser[0].childNodes.length-1].remove();
                }        
            } else {
                ...
                $('#' + fieldId).slideUp('fast', function () {
                    ...
                    if($("#fieldvalue" + collapser.attr("data-fieldvalue-container-id").substring(20) + "-description").val())
                        collapser.append($("#fieldvalue" + collapser.attr("data-fieldvalue-container-id").substring(20) + "-description").val());
                    else if($("#fieldvalue" + collapser.attr("data-fieldvalue-container-id").substring(20) + "-key").val())
                        collapser.append("key:" + $("#fieldvalue" + collapser.attr("data-fieldvalue-container-id").substring(20) + "-key").val());
                    else if($("#fieldvalue" + collapser.attr("data-fieldvalue-container-id").substring(20) + "-value").val())
                        collapser.append("value:" + $("#fieldvalue" + collapser.attr("data-fieldvalue-container-id").substring(20) + "-value").val());
...
/*        var editBar = $('<div></div>').
        ...
        var nullBtn = $('<button></button>').
            addClass('field-add').
            addClass('item-add').
            addClass('invisible').
            text(this.tr("Add"));
        inner.append(' ').append(nullBtn);
...
onde.Onde.prototype.renderObjectPropertyField = function (namespace, baseId, fieldInfo, propName, valueData) {
    ...
    if (labelN.hasClass('collapser')) {
        ...
        var nullBtn = $('<button></button>').
            ...
            addClass('invisible').
            text(this.tr("NULL"));
        labelN.append(nullBtn);
        var pasteBtn = $('<button></button>').
            addClass('field-add').
            addClass('item-paste').
            addClass('right').
            text(this.tr("Paste"));
        labelN.append(pasteBtn);
        var copyBtn = $('<button></button>').
            addClass('field-add').
            addClass('item-copy').
            addClass('right').
            text(this.tr("Copy"));
        labelN.append(copyBtn);
    ...
    if(fieldType == 'array'){
        ...
        this.renderEditBarContent(itemTypes, fieldValueId, rowN, addItemBtn);
        labelN.append(addItemBtn);
...
onde.Onde.prototype.renderListItemField = function (namespace, fieldInfo, index, valueData) {
    var itemId = index;
    /*var itemId = this._generateFieldId();*/
    ...
    if (fieldInfo.type == 'object' && fieldInfo.display == 'inline') {
        ...
        if (collectionType) {
            ...
            var copyBtn = $('<button></button>').
                ...
                text(this.tr("Copy"));
            labelN.append(copyBtn);
            var addBelowBtn = $('<button></button>').
                addClass('field-add').
                addClass('item-add').
                addClass('right').
                attr('op-type', 'addBelow').
                attr('data-cur-id', fieldId).
                attr('data-field-id', 'fieldvalue-' + this._fieldNameToID(namespace)).
                attr('data-object-namespace', namespace).
                text(this.tr("Add Below"));
            labelN.append(addBelowBtn);
            this.renderEditBarContent([fieldInfo], 'fieldvalue-' + this._fieldNameToID(namespace), rowN, addBelowBtn);
            var addAboveBtn = $('<button></button>').
                addClass('field-add').
                addClass('item-add').
                addClass('right').
                attr('op-type', 'addAbove').
                attr('data-cur-id', fieldId).
                attr('data-field-id', 'fieldvalue-' + this._fieldNameToID(namespace)).
                attr('data-object-namespace', namespace).
                text(this.tr("Add Above"));
            labelN.append(addAboveBtn);
            this.renderEditBarContent([fieldInfo], 'fieldvalue-' + this._fieldNameToID(namespace), rowN, addAboveBtn);
...
/*    var lastIndex = parseInt(handle.attr('data-last-index'), 10) + 1;
    ...
    var curId = handle.attr('data-cur-id');
    var namespace = handle.attr('data-object-namespace');
    var ftype = handle.attr('data-object-type') || $('#' + baseId + '-type').val();
    var fieldInfo = this._getFieldInfo(handle);
    var opType = handle.attr('op-type');
    if (!fieldInfo) {
        // No schema found, build it
    ...
    var lastIndex = baseNode.attr('data-last-index');
    if(typeof lastIndex === typeof undefined || lastIndex === false){
        baseNode.attr('data-last-index', '0');
    }
    lastIndex = parseInt(Math.random() * 1000000, 10);//parseInt(baseNode.attr('data-last-index'), 10) + 1;
    baseNode.attr('data-last-index', lastIndex);

    var rowN = this.renderListItemField(namespace, fieldInfo, lastIndex, null);
...
/*    if (siblings.length == 0) {
    ...
    else if(opType == 'addAbove'){
        rowN.insertBefore('#' + curId);
    }
    else if(opType == 'addBelow'){
        rowN.insertAfter('#' + curId);
    }
...
onde.Onde.prototype._buildProperty = function (propName, propInfo, path, formData) {
    ...
    if (dataType == 'object') {
        ...
        for (var i=0; i<formData.size();i++) {
            var fname = formData.array[i];
            if (fname.startsWith(baseFieldName)) {
                
                if(!tmpIdx[fname.slice(baseFieldName.length).split(']', 1)[0]]){
                    itemIndices.push(parseInt(fname.slice(baseFieldName.length).split(']', 1)[0],10));
                }
       ...
       /* for (var fname in tmpIdx) {
        ...
        var valData = null;
        if (formData) {
            valData = formData.get(fieldName);
        }
        if (dataType == 'boolean') {
...
onde.Onde.prototype._buildObject = function (schema, path, formData) {
    ...
    if (!schema || schema.additionalProperties || !schema.properties) {
        ...
        for (var i=0; i<formData.size();i++) {
            var fieldName = formData.array[i];
            var valData = formData.get(fieldName);
            if (!valData) {
                continue;
            }
